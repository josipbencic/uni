...
**Zadatak 2**. Učitati iz datoteke niz realnih pozitivnih brojeva te ih pomoću back\_insertera  ubaciti u vektor
i zatim ispisati u tri kolone za svaki učitani x: ` x ln(x) exp(x)`. Ako se učita negativni broj izbaciti izuzetak
(`runtime_error`).
...
iz druge liste koji se nalaze između elemenata 4 i 7. Koristite 
...
provjeravajte jesu li elementi nađeni.
